        ...
        <Container maxWidth="lg">
          <Grid container spacing={4} alignItems="center">
              ...
              <motion.div variants={heroContainerVariants} initial="hidden" animate="visible">
                ...
                <motion.div variants={heroItemVariants}>
                  <Stack direction="row" spacing={2}>
                    <Button component={Link} href="/request-service" variant="contained" color="primary" size="large" sx={{ py: 1.5, px: 4, fontWeight: 700, borderRadius: 2 }}>
                      احصل على تسعيرة فورية
                    </Button>
                    <Button variant="outlined" color="inherit" size="large" component="a" href={`tel:${settings.phone}`} sx={{ py: 1.5, px: 4, fontWeight: 700, borderRadius: 2 }}>
                      اتصل بنا الآن
                    </Button>
                  </Stack>
                </motion.div>
            ...
            <Grid item xs={12} md={5} sx={{ display: { xs: 'none', md: 'block' } }}>
              <motion.div initial={{ opacity: 0, scale: 0.7, rotate: -10 }} animate={{ opacity: 1, scale: 1, rotate: 0 }} transition={{ duration: 0.9, delay: 0.3, ease: 'backOut' }}>
            ...
            <Grid item xs={12} md={6}>
              <ScrollReveal direction="left">
                ...
                <Typography variant="h2" fontWeight={700} sx={{ mt: 1, mb: 3 }}>
                  شركة الحور لنقل العفش والأثاث المنزلي والمكتبي
                </Typography>
                <Typography variant="body1" paragraph color="text.secondary" sx={{ lineHeight: 1.8, fontSize: '1.05rem' }}>
                  نحن في شركة الحور نفتخر بتقديم أفضل خدمات النقل المتكاملة في جميع محافظات دولة الكويت. نلبي تطلعات عملائنا من خلال الاعتماد على أحدث الأدوات التكنولوجية والخبرات الطويلة في هذا المجال.
                </Typography>
                <Typography variant="body1" paragraph color="text.secondary" sx={{ lineHeight: 1.8, fontSize: '1.05rem' }}>
                  مهمتنا هي الحفاظ على سلامة ممتلكاتك الثمينة ونقلها بدون أي خدوش أو تلفيات.
                </Typography>
              </ScrollReveal>
            </Grid>
            <Grid item xs={12} md={6}>
            ...
            {services.map((service, idx) => (
              <Grid item xs={12} sm={6} md={4} key={service.id}>
                <ScrollReveal direction="up" delay={idx * 0.1}>
                  <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', borderRadius: 3, overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', transition: 'transform 0.3s, box-shadow 0.3s', '&:hover': { transform: 'translateY(-6px)', boxShadow: '0 12px 30px rgba(0,0,0,0.12)' } }}>
                    ...
                    <CardContent sx={{ flexGrow: 1 }}>
                      ...
                      <Typography variant="body2" color="text.secondary" sx={{ lineHeight: 1.6, mb: 3 }}>{service.short_description}</Typography>
                    </CardContent>
                    <Box sx={{ p: 2, pt: 0 }}>
                      <Button component={Link} href={`/services/${service.slug}`} variant="text" color="primary" endIcon={<ArrowRight size={16} />} sx={{ fontWeight: 700 }}>
                        اقرأ المزيد
                      </Button>
                    </Box>
                  </Card>
        ...
        <Container maxWidth="lg">
          <ScrollReveal direction="fade">
            <Box textAlign="center" mb={8}>
              ...
              <Typography variant="h2" fontWeight={800} mt={1}>لماذا تختار شركة الحور لنقل العفش؟</Typography>
            </Box>
          </ScrollReveal>
        ...
        <Container maxWidth="lg">
          <ScrollReveal direction="up">
            <Box display="flex" justifyContent="space-between" alignItems="flex-end" mb={6}>
              ...
              <Button component={Link} href="/gallery" variant="outlined" color="primary" sx={{ display: { xs: 'none', sm: 'inline-flex' } }}>
                عرض المعرض بالكامل
              </Button>
            </Box>
          </ScrollReveal>
            ...
            {gallery.slice(0, 6).map((img, idx) => (
              <Grid item xs={12} sm={6} md={4} key={img.id}>
                <ScrollReveal direction="up" delay={idx * 0.08}>
                  <Card sx={{ borderRadius: 3, overflow: 'hidden', transition: 'transform 0.3s', '&:hover': { transform: 'scale(1.03)' } }}>
                  ...
                  <Card sx={{ height: '100%', bgcolor: '#f8fafc', p: 1, borderRadius: 3, border: '1px solid #f1f5f9' }}>
                    <CardContent>
                      ...
                      <Stack direction="row" alignItems="center" spacing={2} mt={3}>
                        {test.image && <Box component="img" src={test.image} sx={{ width: 44, height: 44, borderRadius: '50%', objectFit: 'cover' }} />}
                        <Box>
        ...
        <Container maxWidth="lg">
          <ScrollReveal direction="up">
            <Box display="flex" justifyContent="space-between" alignItems="flex-end" mb={6}>
              ...
              <Button component={Link} href="/blog" variant="outlined" color="primary">زيارة المدونة</Button>
            </Box>
          </ScrollReveal>
          <Grid container spacing={3}>
            {articles.map((art, idx) => (
              <Grid item xs={12} md={4} key={art.id}>
                <ScrollReveal direction="up" delay={idx * 0.12}>
                  <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', borderRadius: 3, overflow: 'hidden', transition: 'transform 0.3s', '&:hover': { transform: 'translateY(-5px)' } }}>
                    ...
                    <Box p={2}>
                      <Button component={Link} href={`/blog/${art.slug}`} variant="text" color="primary" sx={{ fontWeight: 700 }}>
                        اقرأ المقال كاملاً
                      </Button>
                    </Box>
                  </Card>
              ...
              <ScrollReveal direction="right">
                <Card sx={{ borderRadius: 3, p: 2 }}>
                  <CardContent>
                    ...
                    {formSuccess && <Alert severity="success" sx={{ mb: 3 }}>تم إرسال طلبك بنجاح! سنتواصل معك قريباً جداً.</Alert>}
                    {formError && <Alert severity="error" sx={{ mb: 3 }}>{formError}</Alert>}
                    <Box component="form" noValidate onSubmit={formik.handleSubmit}>
                      <TextField fullWidth name="name" label="اسمك الكريم" value={formik.values.name} onChange={formik.handleChange} error={formik.touched.name && Boolean(formik.errors.name)} helperText={formik.touched.name && formik.errors.name} sx={{ mb: 2.5 }} />
                      <TextField fullWidth name="phone" label="رقم الهاتف (الخلوي)" value={formik.values.phone} onChange={formik.handleChange} error={formik.touched.phone && Boolean(formik.errors.phone)} helperText={formik.touched.phone && formik.errors.phone} sx={{ mb: 2.5 }} />
                      <TextField fullWidth name="service_id" select label="اختر نوع الخدمة المطلوبة" value={formik.values.service_id} onChange={formik.handleChange} error={formik.touched.service_id && Boolean(formik.errors.service_id)} helperText={formik.touched.service_id && formik.errors.service_id} sx={{ mb: 2.5 }}>
                        <MenuItem value="0">طلب نقل عام (عفش وأثاث)</MenuItem>
                        {services.map((service) => (
                          <MenuItem key={service.id} value={service.id}>{service.title}</MenuItem>
                        ))}
                      </TextField>
                      <TextField fullWidth name="message" label="تفاصيل النقل (المنطقة الحالية، وجهة النقل، تفاصيل الأثاث)" multiline rows={4} value={formik.values.message} onChange={formik.handleChange} error={formik.touched.message && Boolean(formik.errors.message)} helperText={formik.touched.message && formik.errors.message} sx={{ mb: 3 }} />
                      <Button fullWidth type="submit" variant="contained" color="primary" size="large" disabled={formSubmitting} sx={{ py: 1.5, fontWeight: 700, borderRadius: 2 }}>
