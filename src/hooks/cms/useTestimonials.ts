...
export function useTestimonials(initialFilters: TableFilters = {}) {
  ...
  const { data, error, isLoading, mutate } = useSWR(['testimonials', filters], () => testimonialsApi.getAll(filters), {
...
export function useMutateTestimonial() {
  ...
  const create = async (values: TestimonialFormValues) => {
    ...
    setError(null);
    try {
      return await testimonialsApi.create(values);
    } catch (e: any) {
      setError(e.message);
      throw e;
    } finally {
  ...
  const update = async (id: number, values: TestimonialFormValues) => {
    ...
    setError(null);
    try {
      return await testimonialsApi.update(id, values);
    } catch (e: any) {
      setError(e.message);
      throw e;
    } finally {
